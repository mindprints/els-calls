...
class AIConversation:
    ...
    def speech_to_text(self, audio_url: str) -> Optional[str]:
        ...
        if not self.soniox_api_key:
            print("❌ Soniox API key not configured", flush=True)
            return None
        ...
        try:
            ...
            print(f"📝 Created transcription: {transcription_id}", flush=True)
            ...
            for attempt in range(max_attempts):
                ...
                if status == "completed":
                    ...
                    if "text" in transcript_data:
                        ...
                        print(f"🎙️  STT Result: {text}", flush=True)
                        return text
                    else:
                        print("❌ No transcription text in result", flush=True)
                        return None
                elif status == "error":
                    print(f"❌ Transcription failed: {status_data.get('error_message', 'Unknown error')}", flush=True)
                    return None
                print(f"⏳ Transcription status: {status} (attempt {attempt + 1}/{max_attempts})", flush=True)
    ...
    def generate_response(
        ...
        if not self.deepseek_api_key:
            print("❌ DeepSeek API key not configured", flush=True)
            return None
        try:
    ...
    def text_to_speech(
        ...
        try:
            ...
            print(f"🔊 TTS saved: {filename}", flush=True)
            return str(filepath)
        except Exception as e:
...
def calls():
    try:
        print(f"📞 Incoming call request: {dict(request.forms)}", flush=True)
        
        from_number = (request.forms.get("from") or "").replace(" ", "")
        if from_number != settings.get("MIL_NUMBER"):
            print(f"Redirecting {from_number} to fallback", flush=True)
        ...
        print(f"Action determined: {action}", flush=True)
        ...
        print(f"❌ Error in calls endpoint: {e}", flush=True)
